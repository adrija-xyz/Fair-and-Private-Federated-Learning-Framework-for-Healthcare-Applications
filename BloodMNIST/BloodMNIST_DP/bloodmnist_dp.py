...
SCENARIO = "BloodMNIST-FedCIR-MAB-KL-DP"
RESULTS_DIR = "./results"
...
def compute_class_distribution(model):
    ...
    for data, target in aux_loader:
        ...
        model.zero_grad()
        output = self_model_forward(model, data)
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config=None):
        ...
        for data, target in self.train_loader:
            target = target.squeeze().long()
            opt.zero_grad()
            out = self_model_forward(self.model, data) if hasattr(self, "self_model_forward") else self.model(data)
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for data, target in DataLoader(test_dataset, batch_size=64, shuffle=False):
            target = target.squeeze().long()
            out = self_model_forward(global_model, data)
            loss_sum += F.cross_entropy(out, target, reduction='sum').item()
            pred = out.argmax(dim=1)
            y_true.extend(target.cpu().numpy().tolist())
            y_pred.extend(pred.cpu().numpy().tolist())
