        ...
        if self.client_distributions:
            ...
            for cl in range(self.num_clusters):
                ...
                if np.random.rand() < self.epsilon:
                    ...
                    # larger reward = more likely to choose
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config=None):
        ...
        # Global rarity weights from (running) sum; keep simple for demo
        present = np.where(local_class_counts > 0)[0]
        w = 1.0 / (local_class_counts.sum() + 1e-6)  # simple placeholder weight
        rare_score = float(len(present)) * w  # presence-based component

        R = compute_class_distribution(self.model)
        kl = compute_kl_divergence(R)
        lam = self.strategy.lambda_weight
        reward = lam * kl + (1 - lam) * rare_score
        ...
        # Return raw parameters (no DP noise/clipping)
