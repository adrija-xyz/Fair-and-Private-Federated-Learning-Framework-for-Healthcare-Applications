...
class BaselineRandomFedAvg(FedAvg):
    """
    FedAvg wrapper that:
      - randomly samples clients each round (no MAB/clustering/rewards),
      - tracks per-client participation,
      - stores final params for post-hoc eval.
    """
    ...
    def configure_fit(self, server_round, parameters, client_manager):
        # Uniform random sampling handled by client_manager.sample
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config=None):
        ...
        # FedProx-style proximal term (small mu)
        mu = 0.01
        ...
        # ---- DP step: clip update L2 norm and add Gaussian noise (same as your code) ----
