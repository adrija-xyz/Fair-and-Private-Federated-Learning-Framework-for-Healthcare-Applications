...
# ---------------------------- Run tag & outputs ---------------------------- #
SCENARIO = "Baseline-Random-DP"  # label used in saved CSVs
RESULTS_DIR = "./results"
...
# (Optional) hybrid partition; unused by default here
...
class BaselineRandomDP(FedAvg):
    ...
    def __init__(self, noise_std=0.001, max_clip_norm=5.0, **kwargs):
        ...
        self.max_clip_norm = float(max_clip_norm)
...
class FlowerClient(fl.client.NumPyClient):
    def __init__(self, cid, model, train_loader, strategy: BaselineRandomDP):
        ...
        self.strategy = strategy  # gives access to DP params
    ...
    def fit(self, parameters, config=None):
        ...
        # FedProx-like proximal term to stabilize (independent of DP)
        ...
        # ----- Differential Privacy: clip update norm and add Gaussian noise -----
        updated_params = self.get_parameters()

        # Compute L2 norm over the concatenated parameter vector
...
if __name__ == "__main__":
    strategy = BaselineRandomDP(
        fraction_fit=0.2,             # same cadence as other runs
        min_fit_clients=2,
        min_available_clients=num_clients,
        fraction_evaluate=0.2,
        min_evaluate_clients=2,
        noise_std=0.001,              # DP noise std (tune per privacy/utility needs)
        max_clip_norm=5.0,            # DP clipping threshold
    )
