...
# ---------------------------- Run tag & outputs ---------------------------- #
SCENARIO = "Non-IID"  # <-- change per run: "IID" / "Non-IID" / "Baseline"
RESULTS_DIR = "./results"
...
# small aux set for fast eval on clients
aux_indices = []
...
class FedCIR_MAB_KL(FedAvg):
    def __init__(self, num_clusters=5, epsilon=0.1, lambda_weight=0.7, **kwargs):
    ...
    # Send server_round to clients so they can tag their CSV rows
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config=None):
        ...
        # compute class-sensitivity distribution + reward
        ...
        # record stats for MAB selection
        ...
        # return plain updated params (no DP)
...
if __name__ == "__main__":
    strategy = FedCIR_MAB_KL(
        ...
        epsilon=0.05,
        lambda_weight=0.7,
        fraction_evaluate=0.2,       # ensure clients are evaluated each round
